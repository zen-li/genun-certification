...
pub fn query_metadata<R>(f: impl FnOnce(&CollectionMetadata) -> R) -> R {
    COLLECTION_METADATA.with(|metadata_cell| {
        ...
        let metadata = metadata_ref.get();
        ic_cdk::println!("query_metadata: {:?}", metadata); // Debug log
        f(metadata)
    })
}
...
pub fn update_metadata<F: FnOnce(&mut CollectionMetadata)>(f: F) {
    COLLECTION_METADATA.with(|metadata_cell| {
        let mut metadata = metadata_cell.borrow_mut();
        let mut data = metadata.get().clone();
        ic_cdk::println!("update_metadata before: {:?}", data); // Debug log
        f(&mut data);
        ic_cdk::println!("update_metadata after: {:?}", data); // Debug log
        metadata.set(data).expect("Failed to update metadata");
    });
}
